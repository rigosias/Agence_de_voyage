...
@extends('layouts.app')
@section('content')
    <form id="form-submit" action="{{route('vols')}}" method="post">
    <table class="table table-bordered" style=" width: 95%; margin-left: 30px ">
        {{ csrf_field() }}
        <thead>
        <tr>
            <th scope="col" colspan="12">Flights</th>
        ...
        <tr>
            ...
            <th scope="col" colspan="">Code Avion</th>
        ...
        <tbody>
        @foreach($vols as $info_vol)
            <tr>
                <th scope="row">{{ $loop->iteration }}</th>
                <td colspan="">{{$info_vol->avo_code_vol}}</td>
                <td colspan="">{{$info_vol->avo_code_avion}}</td>
                ...
                @if(($info_vol->Type_itn == "Round Trip" && $loop->iteration%2 != 0) || ($info_vol->Type_itn == "One Way"))
                    ...
                    <input type="hidden" name="code_avion" value={{$info_vol->avo_code_avion}}>
                    ...
                    <input type="hidden" name="date_arrivee" value={{\Carbon\Carbon::parse($info_vol->avo_date_arrivee)->format('d-m-y')}}>
                    ...
                    <td><button type='submit' class='btn btn-success'><i class="fa-plane" aria-hidden="true"></i>{{"Acheter"}}</button></td>
                @else
                    <td></td>
                @endif
            </tr>
